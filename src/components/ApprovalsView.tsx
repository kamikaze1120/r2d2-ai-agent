    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Tasks below the confidence threshold are queued here for human review.
        </p>
      </div>
          ...
          <div className="divide-y divide-border">
            ...
            {tasks.map((t) => {
              const result = (typeof t.result === "object" ? t.result : null) as
                | { reason?: string; listing_preview?: { title?: string; description?: string; tags?: string[]; price_usd?: number } }
                | null;
              const preview = result?.listing_preview;
              return (
                <div key={t.id} className="space-y-2 p-4 text-sm">
